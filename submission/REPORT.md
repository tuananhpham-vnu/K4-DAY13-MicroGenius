...
- Link/đường dẫn dashboard: Langfuse project `My Project` — `cloud.langfuse.com/project/cmsoct2jl00dfad0d8280qjs8`. Kết quả `python scripts/validate_dashboard.py`: `HỢP LỆ: 6/6 panel có trong dashboard contract.`
  - ⚠️ *Cần bổ sung*: một ảnh dashboard runtime (6 panel: latency, traffic, errors, cost, tokens, quality) theo `config/dashboard.yaml` — hiện `submission/evidence/` chỉ có ảnh trace list và metrics JSON thô, chưa có ảnh dashboard đã dựng (Langfuse Dashboards / Streamlit / Grafana...). Xem `docs/DASHBOARD_SETUP.md`.
...
- Evidence dashboard: *(cần bổ sung ảnh dashboard runtime — xem ghi chú ở mục 2)*
...
> ⚠️ *Phần này chưa có evidence — challenge chưa được chạy trong `data/logs.jsonl` hiện tại (không tìm thấy session `k4-challenge-*`). Cần chạy `python scripts/load_test.py --challenge` rồi điều tra theo checklist bên dưới.*
